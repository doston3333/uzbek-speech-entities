...
def _service(**kwargs: object) -> TransformersSpeechToTextService:
    return TransformersSpeechToTextService("model", Path("cache"), **kwargs)
...
def test_model_loads_once_uses_mps_and_passes_explicit_whisper_options(
    ...
    monkeypatch.setattr("uzbek_speech_entities.stt.transformers_backend.decode_audio", lambda _path: _decoded())
...
def test_audio_longer_than_the_native_chunk_uses_bounded_short_form_chunks(
    ...
    assert all(
        call["return_timestamps"] is False for call in _FakeModel.generate_calls
    )
...
@pytest.mark.parametrize(
    "kwargs",
    [
        {"model_id": "", "cache_dir": Path("cache")},
        {"model_id": "model", "cache_dir": Path("cache"), "chunk_length_seconds": 0},
        {"model_id": "model", "cache_dir": Path("cache"), "batch_size": 2},
        {"model_id": "model", "cache_dir": Path("cache"), "device_preference": ("cuda",)},
    ],
)
...
def test_transcribe_rejects_noncanonical_or_invalid_audio(
    ...
    service = _service()
    monkeypatch.setattr(service, "_load_dependencies", lambda: _dependencies(True))
    monkeypatch.setattr("uzbek_speech_entities.stt.transformers_backend.decode_audio", lambda _path: decoded)
...
@pytest.mark.parametrize("mutator", ["batch", "device", "language"])
